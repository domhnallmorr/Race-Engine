...
class TimingScreen(customtkinter.CTkFrame):
	...
	def setup_labels(self):
		...
		self.lap_label = customtkinter.CTkLabel(self.top_frame, text="LAP 1/5")
	...
	def setup_buttons(self):
		self.start_btn = customtkinter.CTkButton(self.top_frame, text="Start Race", command=self.start_race)
		self.start_btn.grid(row=0, column=1, padx=self.view.padx, pady=self.view.pady, sticky="NW")

		self.timing_button = customtkinter.CTkButton(self.button_frame, text="Timing", command=lambda window="timing": self.show_window(window))
		self.timing_button.grid(row=0, column=0, sticky="EW", padx=self.view.padx, pady=self.view.pady)

		self.lap_times_button = customtkinter.CTkButton(self.button_frame, text="Lap Times", command=lambda window="laptimes": self.show_window(window))
	...
	def setup_widgets(self):
		...
		# LAP TIMES COMBOS
		self.driver1_laptime_combo = customtkinter.CTkComboBox(self.laptimes_frame, values=self.view.driver_names)
		...
		self.driver2_laptime_checkbox = customtkinter.CTkCheckBox(self.laptimes_frame, text="", variable=self.driver2_laptime_var, onvalue="on", offvalue="off", width=10)
		self.driver2_laptime_checkbox.grid(row=1, column=2, sticky="E", padx=(120, 5), pady=self.view.pady)

		self.driver2_laptime_combo = customtkinter.CTkComboBox(self.laptimes_frame, values=self.view.driver_names)
	...
	def start_race(self):
		self.start_btn.configure(text="Pause", command=self.view.controller.pause_resume)
		self.view.controller.start_race()

	def update_view(self, data):
		self.lap_label.configure(text=f"{data['current_lap']}/{data['total_laps']}")
		...
		self.table.update(data["standings"], data["fastest_lap_times"], data["retirements"])

		self.commentary_label.configure(text=data["commentary"])
		self.driver1_fuel_label.configure(text=f'{data["driver1_fuel"]}kg')

		self.update_laptimes_plot(data["laptimes"], data["current_lap"])
	...
	def update_laptimes_plot(self, laptimes_data, current_lap):
		self.laptimes_axis.cla()

		laptimes = laptimes_data[self.driver1_laptime_combo.get()]
		...
		if self.driver2_laptime_var.get() == "on":
			laptimes = laptimes_data[self.driver2_laptime_combo.get()]
			laps = [i for i in range(len(laptimes))]
